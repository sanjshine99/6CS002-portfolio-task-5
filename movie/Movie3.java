...
/******************************************************************************
 * This class demonstrates how to calculate the minimum budget among movies.
 ******************************************************************************/
...
public class Movie3 {
    public static void main(String[] args) {
        ...
        movies.forEach(movie -> System.out.printf("%-20s%.2f\n", movie.getName(), movie.getBudget()));

        // Calculate the minimum budget among movies
        Optional<Movie> minBudgetMovie = movies.stream().min(Comparator.comparingDouble(Movie::getBudget));

        // Check if a movie with a minimum budget is present
        if (minBudgetMovie.isPresent()) {
            // Get the movie with the minimum budget
            Movie movieWithMinBudget = minBudgetMovie.get();

            // Print the result including the movie name
            System.out.printf("\nLowest budget among movies is %.2f for %s\n",
                    movieWithMinBudget.getBudget(), movieWithMinBudget.getName());
        } else {
            // Handle the case where no budget information is available
