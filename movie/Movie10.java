...
/******************************************************************************
 * Example: Filtering, mapping, and collecting movies in a single pipeline
 * This class demonstrates the use of functional programming to process a list
 * of movies. It filters movies based on the genre "Action," extracts the stars
 * of those movies, and collects the star names into a list. This showcases the
 * concise and expressive nature of Java streams and lambda expressions for
 * performing multiple operations in a single pipeline.
 ******************************************************************************/
...
public class Movie10 {
    public static void main(String[] args) {
        ...
        // Example: Filter, map, and collect in a single pipeline
        List<String> actionMovieStars = movies.stream()
                .filter(movie -> "Action".equals(movie.getGenre()))
                .map(Movie::getStar)
                .collect(Collectors.toList());

        // Check if the list is empty
        if (actionMovieStars.isEmpty()) {
            System.out.println("No Action Movie Stars found.");
        } else {
            // Print the list of sci-fi movie stars
            System.out.println("Action Movie Stars:");
            actionMovieStars.forEach(System.out::println);
        }
    }
}
